# 👨‍💻 David Prades - Mobile Developer

¡Bienvenido a mi perfil de desarrollador! Soy David Prades, creador de aplicaciones móviles que buscan ofrecer experiencias útiles y agradables a los usuarios.

## 🚀 Sobre mí

Desarrollador apasionado por crear aplicaciones móviles intuitivas y funcionales. Mi enfoque es construir productos que resuelvan problemas reales con interfaces limpias y un rendimiento optimizado.

- 📱 Especializado en desarrollo móvil multiplataforma
- 🎨 Amante del diseño UI/UX
- 🔧 Siempre aprendiendo nuevas tecnologías
- 🌍 Basado en Barcelona, España

## 📱 Mis Aplicaciones

### 🍣 [Sushi Counter](https://play.google.com/store/apps/details?id=com.davidprades.sushicounter)
**Contador de Comida Interactivo**

Una aplicación elegante para llevar el registro de tu comida favorita. Perfecta para amantes del sushi y la comida japonesa.

**Características destacadas:**
- 🎨 5 temas de colores personalizables (incluyendo temas premium)
- 📊 Estadísticas e historial detallado
- 🍱 Múltiples tipos de comida japonesa (nigiri, maki, ramen, gyoza, etc.)
- 🌍 Soporte multiidioma (Español e Inglés)
- 💾 Sistema de guardado automático
- 📥 Exportación de datos a CSV
- 🎵 Efectos de sonido interactivos
- ✨ Animaciones fluidas y atractivas

**Tecnologías utilizadas:**
- Angular 19
- TypeScript
- Capacitor 7
- AdMob para monetización
- SCSS para estilos

[![Disponible en Google Play](https://play.google.com/intl/en_us/badges/static/images/badges/es_badge_web_generic.png)](https://play.google.com/store/apps/details?id=com.davidpy.sushicounter)

---

### 🚀 Próximos Proyectos
Estoy trabajando en nuevas aplicaciones emocionantes. ¡Mantente atento!

## 💻 Stack Tecnológico

### Desarrollo Móvil
- **Frameworks:** Angular, Ionic, Capacitor
- **Lenguajes:** TypeScript, JavaScript
- **Platforms:** Android, iOS, Web (PWA)

### Frontend
- HTML5, CSS3, SCSS/SASS
- Responsive Design
- Animaciones y transiciones
- UI/UX Design

### Herramientas & Servicios
- Git & GitHub
- Android Studio
- Xcode
- VS Code
- npm & Node.js
- Google Play Console
- AdMob
- Firebase

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=davidprades&show_icons=true&theme=radical)

## 📫 Contacto

¿Tienes alguna pregunta o propuesta? ¡Me encantaría escucharte!

- 📧 Email: [davidprades8@gmail.com](mailto:davidprades8@gmail.com)
- 🐙 GitHub: [@davidprades](https://github.com/davidprades)
- 📱 Google Play: [Ver mis apps](https://play.google.com/store/apps/developer?id=davidpy)
- 🌐 Web: [davidprades.github.io](https://davidprades.github.io)

## 🎯 Objetivos 2026

- [ ] Publicar 3 nuevas aplicaciones en Google Play
- [ ] Alcanzar 10,000 descargas totales
- [ ] Contribuir a proyectos open source
- [ ] Mejorar skills en diseño UI/UX

## 📝 Licencia

Todos mis proyectos públicos están disponibles bajo la licencia MIT, a menos que se especifique lo contrario.

---

⭐️ **¿Te gusta mi trabajo?** ¡Dale una estrella a mis repositorios!

💬 **¿Feedback o sugerencias?** Abre un issue o contáctame directamente.

🚀 **¿Buscas colaborar?** ¡Envíame un mensaje!

---

<div align="center">
  <p>Hecho con ❤️ y mucho ☕</p>
  <p>© 2026 David Prades. Todos los derechos reservados.</p>
</div>
